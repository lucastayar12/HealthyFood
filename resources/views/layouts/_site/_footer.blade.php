<div class="footer">
<footer class="page-footer green darken-2">
          <div class="container">
            <div class="row">
              <div class="col l6 s12">
                <h5 class="white-text">HealthyFood</h5>
                <!-- <p class="grey-text text-lighten-4">Menu</p> -->
                <div class="col-lg-6 text-right d-none d-lg-block">
                <ul class="socials-top">
                  <li><a href="https://pt-br.facebook.com/HealthyFood-103508267776272/" target="_blank"><i class="fa fa-facebook">Facebook</i></a></li>
                  <li><a href="https://twitter.com/kellyroberta" target="_blank"><i class="fa fa-twitter">Twitter</i></a></li>
                  <li><a href="https://www.instagram.com/kellyroberta/?hl=pt-br" target="_blank"><i class="fa fa-instagram">Instagram</i></a></li>
                  <li><a href="https://www.youtube.com/channel/UCTUOCeKtGsjo4xOdUUCqn0Q" target="_blank"><i class="fa fa-youtube-play">Youtube</i></a></li>
                  </ul>
                  </div>
=
              </div>
              <div class="col l4 offset-l2 s12">
                <h5 class="white-text">Links</h5>
                <ul>
                  <li><a class="grey-text text-lighten-3" href="{{route ('site.home')}}">Home</a></li>
                  <li><a class="grey-text text-lighten-3" href="{{route ('site.sobre')}}">Sobre</a></li>
                  <li><a class="grey-text text-lighten-3" href="{{route ('site.contato')}}">Contato</a></li>
                </ul>
              </div>

              <!-- <div class="col l6 s12"> -->
                <div align="center">
                <img src="{{asset('imagens/iftm.png')}}" width="60%" height="60%">
              </div>

            </div>
          </div>
          <div class="footer-copyright">
            <div class="container">
           <div align="center">  Mostra de produtos tecnologicos IFTM 2019 </div>
            <!-- <a class="grey-text text-lighten-4 right" href="#!">.</a> -->
            </div>
          </div>
  </footer>
</div>